...
- `entity QuickOrderBill as projection on lunch.QuickOrderBill`
...
2. Optional: click `Scan Menu`.
3. UI calls scan endpoint and receives `[name, price, description?]` entries.
4. User edits rows.
5. Confirm -> create order with `status='open'`.
